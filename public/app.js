...
function setTab(t, btn) {
    ...
    if (t === 'whitelist') loadWhitelist(); // <--- AÑADIDO WHITELIST
}
...
// === WHITELIST SYSTEM ===
let wlData = [{name:"Admin",date:"Hoy"}]; // Mock inicial

function loadWhitelist() {
    renderWL();
    // En producción: fetch('/api/whitelist').then(r=>r.json()).then(d=>{wlData=d; renderWL()});
}
...
function renderWL() {
    ...
    wlData.forEach(user => {
        ...
        div.innerHTML = `
            <img src="https://minotar.net/helm/${user.name}/36.png" class="wl-avatar" alt="${user.name}">
            <div class="wl-info">
                <div class="wl-name">${user.name}</div>
                <div class="wl-date">${user.date}</div>
            </div>
...
function addWL() {
    ...
    if(wlData.some(u => u.name.toLowerCase() === name.toLowerCase())) {
        Toastify({text: "El usuario ya está", style:{background:"#f59e0b"}}).showToast();
        return;
    }
    
    wlData.unshift({name: name, date: "Ahora"});
    renderWL();
    input.value = '';
    Toastify({text: `Añadido: ${name}`, style:{background:"#10b981"}}).showToast();
    // api('whitelist/add', {user: name});
}

function removeWL(name) {
    wlData = wlData.filter(u => u.name !== name);
    renderWL();
    Toastify({text: `Eliminado: ${name}`, style:{background:"#ef4444"}}).showToast();
    // api('whitelist/remove', {user: name});
}
...
function toggleWL(state) {
    ...
    // api('config/save', {'white-list': state});
}
