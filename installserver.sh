...
# ============================================================
# NEBULA PANEL - V1.2.1 (HYBRID UPDATE + API FIX)
...
echo -e "${PURPLE}🌌 INSTALANDO NEBULA V1.2.1 (HYBRID)...${NC}"
...
cat <<'EOF' > /opt/aetherpanel/package.json
{
  ...
  "version": "1.2.1",
  "description": "Nebula V1.2.1 Hybrid",
  "main": "server.js",
...
app.get('/api/update/check', async (req, res) => {
    try {
        ...
        // 2. Si versión es igual, chequear cambios visuales (HTML/CSS)
        // Descargamos el remoto y comparamos con el local
        const remoteHtml = (await apiClient.get(`${REPO_RAW}/public/index.html`)).data;
        const localHtml = fs.readFileSync(path.join(__dirname, 'public', 'index.html'), 'utf8');
        
        const remoteCss = (await apiClient.get(`${REPO_RAW}/public/style.css`)).data;
        const localCss = fs.readFileSync(path.join(__dirname, 'public', 'style.css'), 'utf8');

        if (remoteHtml !== localHtml || remoteCss !== localCss) {
...
app.post('/api/update/perform', async (req, res) => {
    ...
    if (type === 'full') {
        // ACTUALIZACIÓN COMPLETA (ZIP + RESTART)
        ...
        // HOTFIX (SOLO HTML/CSS, SIN REINICIO)
        io.emit('toast', { type: 'info', msg: '🎨 Aplicando parche visual...' });
        try {
            const html = (await apiClient.get(`${REPO_RAW}/public/index.html`)).data;
            const css = (await apiClient.get(`${REPO_RAW}/public/style.css`)).data;
...
class MCManager {
    ...
    async installJar(url, filename) {
        ...
        try { 
            ...
            const r = await axios({url, method:'GET', responseType:'stream', headers: {'User-Agent': 'Nebula/1.2.1'}}); 
...
<body>
    ...
    <div class="app-layout">
        <aside class="sidebar">
            <div class="brand"><div class="brand-logo"><i class="fa-solid fa-meteor"></i></div><div class="brand-text">V<span id="sidebar-version">...</span> <span>NEBULA</span></div></div>
            <nav>
            ...
            <div id="tab-config" class="tab-content">
                <div class="card glass full">
                    ...
                    <div style="padding:20px;margin-top:20px;border-top:1px solid var(--border)">
                        <button onclick="checkUpdate()" class="btn" style="background:#8b5cf6;width:100%;justify-content:center"><i class="fa-solid fa-rotate"></i> BUSCAR ACTUALIZACIONES</button>
                    </div>
                </div>
            </div>
        </main>
...
fetch('/api/info').then(r=>r.json()).then(d => {
    document.getElementById('sidebar-version').innerText = d.version;
...
function checkUpdate() {
    ...
    fetch('/api/update/check').then(r=>r.json()).then(d => {
        ...
        if(d.type === 'full') {
            ti.innerText = "Actualización de Sistema";
            t.innerText = `Versión local: ${d.local}\nNueva versión: ${d.remote}\n\nSe requiere reinicio completo.`;
            a.innerHTML = `<button onclick="doUpdate('full')" class="btn btn-primary">ACTUALIZAR (${d.remote})</button><button onclick="document.getElementById('update-modal').style.display='none'" class="btn btn-ghost">Cancelar</button>`;
            m.style.display='flex';
        } else if (d.type === 'hotfix') {
            ti.innerText = "Hotfix Visual";
            t.innerText = `Versión ${d.local} detectada con cambios visuales.\nSe actualizará la interfaz sin reiniciar.`;
            a.innerHTML = `<button onclick="doUpdate('hotfix')" class="btn" style="background:#10b981;color:white">APLICAR HOTFIX</button><button onclick="document.getElementById('update-modal').style.display='none'" class="btn btn-ghost">Cancelar</button>`;
            m.style.display='flex';
        } else if (d.type === 'none') {
...
function doUpdate(type) {
    ...
    fetch('/api/update/perform', {
        ...
        if(d.mode === 'hotfix') {
            Toastify({text:'Hotfix aplicado. Recargando...', style:{background:'#10b981'}}).showToast();
